...
| Navigation               | Icon rail plus a sidebar per section (library, agents, presets, projects), fold with ⌘B, ⌘1–⌘4 section shortcuts, theme switch in the rail, library quick views and recent skills |
...
| Skill editor            | Edits existing files only (no create, rename or delete). Edits made outside the app are not tracked, so an update does not ask before replacing them. A local-folder skill shows "Update available" after an in-app edit. Mixed line endings are saved as the file's majority ending. Not in the command palette; agent-folder and project copies cannot be edited. See TODO item 14. |
