...
| Skill editor            | Edits existing files only: creating, renaming or deleting files is not supported. Edits made outside the app are not tracked, so an update does not ask before replacing them. Mixed line endings are saved as the file's majority ending. |
